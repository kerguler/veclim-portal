...
function TileNameDisplay() {
	...
	const displayTileNames = useSelector(
		(state) => state.fetcher.fetcherStates.map.displayTileNames,
	);
	...
	const dividerRef = useRef({
		...
		right:
			((window.innerWidth - dividerPos) / (2 * window.innerWidth)) * 100,
	});
	...
	let lft = leftRef.current
		? leftRef.current.getBoundingClientRect().width
		: 0;
	...
	divider.right =
		((window.innerWidth - dividerPos - rght) / (2 * window.innerWidth)) *
		100;
	const tileLabels = displayTileNames
		? tileArray.map((tile, index) => {
				let selectedTile = tileIcons.find(
	...
	useEffect(() => {
		if (displayTileNames) {
			display.current = {
				left:
					displayTileNames.left && tileLabels.length === 2
						? true
						: false,
				right:
					displayTileNames.right && tileLabels.length === 2
						? true
						: false,
				single:
					displayTileNames.center && tileLabels.length === 1
						? true
						: false,
			};
		} else {
