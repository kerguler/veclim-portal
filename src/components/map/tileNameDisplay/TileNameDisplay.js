...
function TileNameDisplay() {
  ...
  const { tileIcons } = useDirectorFun('left');
  const displayTileNames = useSelector((state) => state.fetcher.fetcherStates.map.displayTileNames);
  ...
  let rght = rightRef.current ? rightRef.current.getBoundingClientRect().width : 0;

  divider.left = ((dividerPos - lft / 2) / (2 * window.innerWidth)) * 100;

  divider.right = ((window.innerWidth - dividerPos - rght) / (2 * window.innerWidth)) * 100;
  ...
  useEffect(() => {
    if (displayTileNames) {
      display.current = {
        ...
        single: displayTileNames.center && tileLabels.length === 1 ? true : false,
      };
    } else {
      display.current = {
  ...
  const rightTitle = (
    <div ref={rightRef} className="right" style={{ left: 'unset', right: `${positions.right}%` }}>
      <p>{tileLabels[1]}</p>
    </div>
  );
  const centerTitle = (
